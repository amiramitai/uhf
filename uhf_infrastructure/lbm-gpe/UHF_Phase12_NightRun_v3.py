#!/usr/bin/env python3
"""UHF Phase 12 Night Run v3: True Merger with Dynamic Lapse
Critical fix: G=0.5 (reduced gravity) so the lapse does NOT immediately
collapse to floor. Lower core amplitudes ensure gradual lapse descent
during inspiral rather than instant freeze.

λ=10, N=80, d_sep=3.0, amp=0.08, sigma=1.8.
...
dx = L / N            # 0.5
dt = 0.0025
G_grav = 0.5          # REDUCED: prevent instant lapse collapse
m_scalar = 0.1
lam = 10.0            # Low GP repulsion
kappa_torsion = 0.8
m_torsion_sq = 1.0
z4c_damping = 0.05
lam_K5_cubic = 0.3
...
# Binary IC — low amplitude for dynamic lapse, close separation for plunge
d_sep = 3.0
sigma_core = 1.8
amp_core = 0.08       # LOW: keeps lapse above floor during inspiral
v_orbit = 0.3
...
# Check initial lapse
rho_init = 0.5 * gradient_sq_magnitude(phi) + 0.5 * m_scalar**2 * torch.abs(phi)**2
V_init = poisson_fft(rho_init)
alpha_init = torch.sqrt(torch.clamp(1.0 + 2.0 * V_init, min=0.01))

print(f"\n{'='*70}")
print(f"  UHF PHASE 12 NIGHT RUN v3: TRUE MERGER (G={G_grav}, lam={lam})")
...
for step in range(n_steps):
    ...
    if step > 0 and step % checkpoint_every == 0:
        with open(f"UHF_results/phase12v3_history_step{step}.json", "w") as f:
            json.dump(history, f)
...
elapsed = time.time() - start_time
print(f"\n{'='*70}")
print(f"  PHASE 12 v3 COMPLETE: {step+1} steps in {elapsed:.1f}s ({elapsed/3600:.2f}h)")
print(f"{'='*70}")

with open("UHF_results/phase12v3_history_final.json", "w") as f:
    json.dump(history, f)

for name, key in [("h_plus","h_plus"),("h_cross","h_cross"),("polarization","circ_pol"),("separation","binary_sep"),("lapse","central_lapse")]:
    with open(f"UHF_results/phase12v3_{name}.csv", "w", newline="") as f:
...
lapse_dynamic = min_lapse_ever > 0.11
merged = min_sep < 2.0
...
axes[1, 1].set_xlabel('Time'); axes[1, 1].set_ylabel('Central Lapse alpha')
axes[1, 1].set_title('Central Lapse Evolution')
...
plt.suptitle(f'UHF Phase 12 v3: True Merger (G={G_grav}, lam={lam}, N={N})', fontsize=15, y=1.01)
plt.tight_layout()
plt.savefig("UHF_results/phase12v3_merger_final.png", dpi=300, bbox_inches='tight')
print(f"Plot saved to UHF_results/phase12v3_merger_final.png")
