#!/usr/bin/env python3
"""
UHF Hydrodynamic General Relativity v1 — Gravitational Lensing
================================================================
2D Klein-Gordon with space-dependent wave speed c(r)²:
    φ_tt = c(r)² ∇²φ - λφ(φ²-η²)

c(r)² = c₀²[1 + δ·exp(-r²/2σ²)]  with δ>0 → faster at centre
                                    → "gravitational attraction"

Background φ=η is an exact static solution for ANY c(r), since
∇²η = 0 and φ(φ²-η²)|_{φ=η} = 0.

A high-k phonon wave-packet refracts through the speed gradient,
producing deflection Δθ(b). We fit Δθ vs b to the power law A/b^α.

GPU: RTX 3090, PyTorch, float64, leapfrog + spectral Laplacian
"""
import torch
import numpy as np
import json, os, time
...
# Speed bump (gravitational lens)
DELTA = 0.6           # fractional speed increase at centre
SIGMA = 5.0           # Gaussian width of the lens
...
# ──── Integration ──────────────────────────────────────────────
DT     = 0.005
T_MAX  = 90.0           # packet traverses ~80ξ at v_g≈1.0c

# ──── Wave packet ──────────────────────────────────────────────────
K0      = 15.0         # carrier wavenumber (λ≈0.42ξ, well-resolved)
W_PKT   = 3.0          # envelope half-width
A_PKT   = 0.02         # small amplitude (linear regime)
X_START = -40.0        # inside sponge-free zone (-50 to +50)
# KG dispersion: ω = √(c₀²k₀² + 2λη²)
OMEGA   = float(np.sqrt(C0**2 * K0**2 + 2 * LAM * ETA**2))  # ≈15.07
...
# ──── Measurement ─────────────────────────────────────────────
MEAS_DT  = 0.5
X_IN_LO  = -38.0       # incoming straight segment (before lens)
X_IN_HI  = -18.0
X_OUT_LO =  18.0       # outgoing straight segment (after lens)
X_OUT_HI =  38.0

OUT_DIR = "UHF_HydroGR_Lensing_v1_results"
...
def build_c2(xx, yy):
    """Space-dependent wave speed squared: c²(r) = c₀²[1 + δ·Gauss(r)]"""
    r2 = xx**2 + yy**2
    return C0**2 * (1.0 + DELTA * torch.exp(-r2 / (2.0 * SIGMA**2)))
...
def init_packet(xx, yy, b):
    ...
    # Rightward KG packet: φ=cos(kx-ωt), φ_t = +ω·sin(kx-ωt)|_{t=0}
...
def make_plots(bs, dthetas, fit_A, fit_alpha, r2_val, traj_data):
    ...
    ax.plot(SIGMA * np.cos(th), SIGMA * np.sin(th), 'k--', alpha=0.4, label=f'σ={SIGMA}')
    ax.set(xlabel='x', ylabel='y', title='Trajectories',
           xlim=(-55, 55), ylim=(-5, 35))
    ax.legend(fontsize=6, ncol=3); ax.grid(alpha=0.3)
    ax.set_aspect('equal')

    fig.suptitle('UHF GR v1 — Phonon Lensing (variable c²)', fontsize=14, y=1.02)
    plt.tight_layout()
    path = os.path.join(OUT_DIR, 'gr_lensing_v1.png')
...
def main():
    print("=" * 70)
    print("  UHF Hydrodynamic GR v1 — Gravitational Lensing")
    print(f"  {NX}x{NY}  L={LX}  dx={LX/NX:.4f}  dt={DT}")
    print(f"  c² bump: δ={DELTA}  σ={SIGMA}  k₀={K0}  w={W_PKT}")
    ...
    print(f"  CFL: dt·c_max/dx = {DT * C0 * np.sqrt(1 + DELTA) / (LX / NX):.3f}")
    ...
    # ── Table ──────────────────────────────────────────────────
    print("\n" + "=" * 55)
    print(f"{'b':>6} {'Δθ':>12} {'1/b':>10}")
    print("-" * 55)
    for b, dth in zip(bs_done, dthetas):
        print(f"{b:6.1f} {dth:12.6f} {1/b:10.6f}")
    print("-" * 55)
    ...
    tol = 0.05
    if not np.isnan(fit_alpha) and abs(fit_alpha - (-1.0)) < tol and r2_val > 0.99:
        conclusion = (f"Deflection scales as 1/b with exponent {fit_alpha:.3f} "
                      f"(target -1.0±{tol}), R²={r2_val:.4f} "
                      f"→ hydrodynamic gravitational lensing confirmed")
    else:
        conclusion = (f"Exponent = {fit_alpha:.3f}, R²={r2_val:.4f} "
                      f"→ Deviation → hypothesis falsified")
    print(f"\n{conclusion}")

    with open(os.path.join(OUT_DIR, 'results_v1.json'), 'w') as f:
        json.dump(dict(
    ...
    print(f"JSON: {OUT_DIR}/results_v1.json")
