#!/usr/bin/env python3
"""UHF Phase 12 Night Run v5b: Parameter-Tuned Merger
Same physics engine as v5 (gauge driver + rest-mass source + grad_alpha force).
Parameter tuning for actual merger:
  - d_sep = 1.5 (overlapping cores)
  - lambda = 1.0 (minimal GP repulsion)
  - amp = 0.15, sigma = 1.5 (stronger, tighter cores)
  - v_orbit = 0.4 (good chirality)
Single RTX 3090.
"""
...
G_grav = 1.0
m_scalar = 0.1
lam = 1.0             # TUNED: minimal GP repulsion
kappa_torsion = 0.8
m_torsion_sq = 1.0
z4c_damping = 0.05
lam_K5_cubic = 0.3
...
# Dynamic lapse parameters (proven in v5)
tau_lapse = 5.0
alpha_target_floor = 0.1
alpha_hard_min = 0.01
alpha_hard_max = 2.0

# Binary IC — TUNED for merger
d_sep = 1.5            # TUNED: half-separation → total = 3 (overlapping at 1.0*sigma)
sigma_core = 1.5       # TUNED: tighter cores
amp_core = 0.15        # TUNED: stronger gravity
v_orbit = 0.4          # TUNED: orbital velocity for chirality
...
print(f"\n{'='*70}")
print(f"  UHF PHASE 12 v5b: PARAMETER-TUNED MERGER")
...
for step in range(n_steps):
    ...
    if step > 0 and step % checkpoint_every == 0:
        with open(f"UHF_results/phase12v5b_history_step{step}.json", "w") as f:
            json.dump(history, f)
...
elapsed = time.time() - start_time
print(f"\n{'='*70}")
print(f"  PHASE 12 v5b COMPLETE: {step+1} steps in {elapsed:.1f}s ({elapsed/3600:.2f}h)")
print(f"{'='*70}")

with open("UHF_results/phase12v5b_history_final.json", "w") as f:
    json.dump(history, f)
...
for name, key in [("h_plus","h_plus"),("h_cross","h_cross"),("polarization","circ_pol"),
    ...
    with open(f"UHF_results/phase12v5b_{name}.csv", "w", newline="") as f:
...
plt.suptitle(f'UHF Phase 12 v5b: Tuned Merger (lam={lam}, d_sep={d_sep}, amp={amp_core})',
             fontsize=14, y=1.01)
plt.tight_layout()
plt.savefig("UHF_results/phase12v5b_merger_final.png", dpi=300, bbox_inches='tight')
print(f"Plot saved to UHF_results/phase12v5b_merger_final.png")
