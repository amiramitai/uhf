#!/usr/bin/env python3
"""UHF Phase 12 Night Run v5: True Merger — Dynamic Lapse Edition
FOUR critical fixes over v4:
  1. Poisson source = |phi|^2 only (no gradient energy feedback)
  2. DYNAMIC lapse: alpha is an evolved field with gauge-driver ODE:
     d_t alpha = -(alpha/tau) * (alpha - alpha_target)
     Singularity avoidance: rate ∝ alpha → alpha can never reach zero.
     tau=5.0 gives relaxation scale ~25 time units at typical alpha.
  3. grad(alpha)·grad(phi) gravitational force in ADM KG evolution
  4. Alpha evolved in RK2 alongside all other fields

Parameters: G=1.0, lambda=5, N=80, d_sep=3.0, amp=0.10, v_orbit=0.3.
...
G_grav = 1.0
m_scalar = 0.1
lam = 5.0             # Lower GP repulsion for merger
kappa_torsion = 0.8
m_torsion_sq = 1.0
z4c_damping = 0.05
lam_K5_cubic = 0.3
...
tau_lapse = 5.0         # gauge driver relaxation timescale
alpha_target_floor = 0.1  # floor for alpha target
alpha_hard_min = 0.01   # absolute safety clamp
alpha_hard_max = 2.0

# Binary IC — closer separation for real plunge
d_sep = 3.0            # half-separation → total = 6
sigma_core = 1.6
amp_core = 0.10
v_orbit = 0.3
...
def compute_rhs(phi_s, pi_s, K5_s, pi_K5_s, alpha_s):
    ...
    # Dynamic lapse ODE: d_t alpha = -(alpha/tau)(alpha - alpha_target)
    # Singularity avoidance: rate ∝ alpha → alpha can never reach zero
...
print(f"\n{'='*70}")
print(f"  UHF PHASE 12 NIGHT RUN v5: DYNAMIC LAPSE MERGER")
print(f"  KEY: alpha evolved with gauge driver ODE (tau={tau_lapse})")
print(f"  d_t alpha = -(alpha/tau)(alpha - alpha_target)")
...
for step in range(n_steps):
    # --- RK2 Midpoint: evolve phi, pi, K5, piK5, alpha together ---
    ...
    # Sponge damping — fields go to zero, alpha relaxes to 1.0
    phi *= sponge_c
    pi_field *= sponge_c
    K5 *= sponge
    pi_K5 *= sponge
    alpha = alpha * sponge + (1.0 - sponge)  # alpha → 1 at boundary
    ...
    if step > 0 and step % checkpoint_every == 0:
        with open(f"UHF_results/phase12v5_history_step{step}.json", "w") as f:
            json.dump(history, f)
...
elapsed = time.time() - start_time
print(f"\n{'='*70}")
print(f"  PHASE 12 v5 COMPLETE: {step+1} steps in {elapsed:.1f}s ({elapsed/3600:.2f}h)")
print(f"{'='*70}")

with open("UHF_results/phase12v5_history_final.json", "w") as f:
    json.dump(history, f)
...
for name, key in [("h_plus","h_plus"),("h_cross","h_cross"),("polarization","circ_pol"),
    ...
    with open(f"UHF_results/phase12v5_{name}.csv", "w", newline="") as f:
...
plt.suptitle(f'UHF Phase 12 v5: Dynamic Lapse Merger (G={G_grav}, lam={lam}, tau={tau_lapse})',
             fontsize=14, y=1.01)
plt.tight_layout()
plt.savefig("UHF_results/phase12v5_merger_final.png", dpi=300, bbox_inches='tight')
print(f"Plot saved to UHF_results/phase12v5_merger_final.png")
