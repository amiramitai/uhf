#!/usr/bin/env python3
"""UHF Phase 12 Night Run v4: True Merger Edition
THREE critical fixes over v3:
  1. Poisson source = |phi|^2 only (no gradient energy → no mass explosion)
  2. 1+log lapse: alpha = clamp(1+2V, min=0.01) (linearized, no sqrt)
  3. NEW: grad(alpha).grad(phi) gravitational force term in evolution
     (correct ADM KG: d_t pi = alpha*D2phi + grad_alpha.grad_phi - alpha*(m^2 phi + ...))

Parameters: G=1.0, lambda=8, N=80, d_sep=4.0, amp=0.12, v_orbit=0.2.
Built on proven Phase 11/12 infrastructure. Single RTX 3090.
"""
...
dx = L / N            # 0.5
dt = 0.0025
G_grav = 1.0          # Full gravity — source term fix prevents mass explosion
m_scalar = 0.1
lam = 8.0             # GP repulsion
kappa_torsion = 0.8
m_torsion_sq = 1.0
z4c_damping = 0.05
lam_K5_cubic = 0.3
...
# Binary IC — moderate amplitude, sub-circular orbit for inspiral
d_sep = 4.0            # half-separation → total = 8
sigma_core = 1.8
amp_core = 0.12        # chosen so V_self ~ -0.29, alpha ~ 0.42 (well above floor)
v_orbit = 0.2          # sub-circular for gradual inspiral
...
def grad_alpha_dot_grad_phi(alpha, phi):
    """Gravitational force: nabla(alpha) . nabla(phi) — correct ADM term.
    alpha is real, phi is complex. Returns complex."""
    result_r = torch.zeros(N, N, N, device=device)
    result_i = torch.zeros(N, N, N, device=device)
    for dim in range(3):
        da = (torch.roll(alpha, -1, dim) - torch.roll(alpha, 1, dim)) / (2 * dx)
        dp_r = (torch.roll(phi.real, -1, dim) - torch.roll(phi.real, 1, dim)) / (2 * dx)
        dp_i = (torch.roll(phi.imag, -1, dim) - torch.roll(phi.imag, 1, dim)) / (2 * dx)
        result_r += da * dp_r
        result_i += da * dp_i
...
def compute_rhs(phi_s, pi_s, K5_s, pi_K5_s):
    ...
    # FIX #1: Rest-mass only source — NO gradient energy feedback
    ...
    # FIX #2: 1+log lapse (linearized) — no sqrt compression
    alpha = torch.clamp(1.0 + 2.0 * V, min=0.01)

    # Torsion-covariant Laplacian
    torsion_grad = gradient_dot_complex(K5_s, phi_s)
    D2_phi = lap_phi + 2j * torsion_grad - K5_s**2 * phi_s

    # FIX #3: Correct ADM evolution with gravitational force term
    # d_t pi = alpha * D2_phi + grad(alpha).grad(phi) - alpha*(m^2 phi + lambda|phi|^2 phi)
    grav_force = grad_alpha_dot_grad_phi(alpha, phi_s)
...
# Check initial lapse with REST-MASS source
rho_init = torch.abs(phi)**2
V_init = poisson_fft(rho_init)
alpha_init = torch.clamp(1.0 + 2.0 * V_init, min=0.01)

print(f"\n{'='*70}")
print(f"  UHF PHASE 12 NIGHT RUN v4: TRUE MERGER EDITION")
print(f"  FIXES: rest-mass source + 1+log lapse + grad(alpha).grad(phi) force")
print(f"  Grid: {N}^3 = {N**3:,} cells | dx={dx:.3f} | dt={dt} | {n_steps:,} steps")
print(f"  Binary: d_sep={d_sep}, sigma={sigma_core}, amp={amp_core}, v_orbit={v_orbit}")
...
print(f"  Initial central lapse: {float(alpha_init[mid,mid,mid]):.4f}")
print(f"  Initial min lapse: {float(alpha_init.min()):.4f}")
print(f"{'='*70}\n")
...
for step in range(n_steps):
    ...
    if step > 0 and step % checkpoint_every == 0:
        with open(f"UHF_results/phase12v4_history_step{step}.json", "w") as f:
            json.dump(history, f)
...
elapsed = time.time() - start_time
print(f"\n{'='*70}")
print(f"  PHASE 12 v4 COMPLETE: {step+1} steps in {elapsed:.1f}s ({elapsed/3600:.2f}h)")
print(f"{'='*70}")

with open("UHF_results/phase12v4_history_final.json", "w") as f:
    json.dump(history, f)
...
for name, key in [("h_plus","h_plus"),("h_cross","h_cross"),("polarization","circ_pol"),
    ...
    with open(f"UHF_results/phase12v4_{name}.csv", "w", newline="") as f:
...
# Check if lapse stayed dynamic (not locked at floor)
...
axes[1, 1].axhline(y=0.10, color='red', ls='--', alpha=0.5, label='Floor (0.10)')
axes[1, 1].set_xlabel('Time'); axes[1, 1].set_ylabel('Lapse alpha')
axes[1, 1].set_title('Lapse Evolution (central + global min)')
...
axes[1, 2].set_title('Total Mass (should stay bounded!)')
...
plt.suptitle(f'UHF Phase 12 v4: True Merger (G={G_grav}, lam={lam}, rest-mass source, 1+log lapse)', fontsize=14, y=1.01)
plt.tight_layout()
plt.savefig("UHF_results/phase12v4_merger_final.png", dpi=300, bbox_inches='tight')
print(f"Plot saved to UHF_results/phase12v4_merger_final.png")
