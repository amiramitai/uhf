...
Architecture:
  Phase 1 — Reference Wavefunction:
    Evolve a wide Gaussian wavepacket with +1 vortex through double-slit barrier.
    30 imaginary-time relaxation steps to form stable pilot-wave structure.
    Real-time evolution stores velocity field snapshots at regular intervals.
    Also records |ψ|² interference envelope at the detection screen.

  Phase 2 — Ensemble Tracking:
    2000 independent Bohmian trajectories, each with:
      - x₀ = −20ξ (at vortex centre)
      - y₀ ~ Uniform(−4ξ, +4ξ) — flat distribution, NOT |ψ|²
    Advected by the pilot-wave velocity field v = Im(ψ*∇ψ)/|ψ|²
    Record y_hit when particle crosses screen at x = +35ξ.

  Analysis:
    Histogram of 2000 y_hit values (100 bins).
    Pearson r and R² vs |ψ|² envelope.
    Fringe spacing fit.
...
# Timesteps
DT_IT    = 0.05       # imaginary-time step
N_IT     = 30         # imaginary-time relaxation steps
...
# Ensemble
N_ENSEMBLE    = 2000
JITTER_RANGE  = 4.0    # y₀ ~ Uniform(-4, +4)
N_BINS        = 100
...
# ══════════════════════════════════════════════════════════════════════
#  Wavepacket + Vortex Initialization
# ══════════════════════════════════════════════════════════════════════
def init_wavepacket_vortex(X, Y, y0, device):
    """Gaussian wavepacket with +1 phase winding centred at (WP_X0, y0)."""
    dx = X - WP_X0
    dy = Y - y0
    ...
    # Momentum kick
    phase_k = WP_KX * X
    # +1 vortex winding at (WP_X0, y0)
    theta_v = torch.atan2(dy, dx)
    phase_total = phase_k + theta_v
    return (envelope * torch.exp(1j * phase_total.to(CDTYPE))).to(CDTYPE)


# ══════════════════════════════════════════════════════════════════════
#  Imaginary-Time Relaxation (phase-locked)
# ══════════════════════════════════════════════════════════════════════
def imaginary_time_relax(psi, X, Y, y0, K2, device):
    """Phase-locked IT relaxation: relax amplitude, restore vortex phase."""
    dt = DT_IT
    kinetic_damp = torch.exp(-0.5 * K2 * dt)

    # Phase target: momentum + vortex
    dx = X - WP_X0
    dy = Y - y0
    phase_target = WP_KX * X + torch.atan2(dy, dx)
    phase_factor = torch.exp(1j * phase_target.to(CDTYPE))

    N_init = torch.sum(torch.abs(psi)**2).item() * DA

    for step in range(N_IT):
        rho = torch.abs(psi)**2
        psi = psi * torch.exp(-dt / 2.0 * (G_NL * rho - MU))

        psi = fft.ifftn(fft.fftn(psi) * kinetic_damp)

        rho = torch.abs(psi)**2
        psi = psi * torch.exp(-dt / 2.0 * (G_NL * rho - MU))

        # Phase lock + renormalize
        amp = torch.abs(psi)
        psi = amp.to(CDTYPE) * phase_factor
        N_now = torch.sum(amp**2).item() * DA
        if N_now > 0:
            psi *= np.sqrt(N_init / N_now)
...
def advect_particles_gpu(vx_field, vy_field, px, py, active, dt_eff):
    ...
    px, py: float64 tensors on GPU (N,).  active: bool tensor on GPU (N,)."""
    # Convert positions to fractional grid indices
    fi = (px + LX/2) / DX
    fj = (py + LY/2) / DY
...
def main():
    ...
    print(f"  Ensemble: N={N_ENSEMBLE}, y₀ ~ Uniform(−{JITTER_RANGE}, "
          f"+{JITTER_RANGE})ξ")
    ...
    # ══════════════════════════════════════════════════════════════════
    psi = init_wavepacket_vortex(X, Y, 0.0, device)

    # Imaginary-time relaxation
    print(f"\n  IT relaxation: {N_IT} steps, dt={DT_IT}...", flush=True)
    psi = imaginary_time_relax(psi, X, Y, 0.0, K2, device)
    norm0 = torch.sum(torch.abs(psi)**2).item() * DA
    print(f"  Post-IT norm = {norm0:.2f}", flush=True)

    # ══════════════════════════════════════════════════════════════════
    #  Evolution parameters
    # ══════════════════════════════════════════════════════════════════
    # Seed particles just past barrier after transmitted wave establishes
    SEED_X = BARRIER_X + BARRIER_THICK / 2.0 + 1.0  # 2ξ past barrier centre
    barrier_arrival = (BARRIER_X - WP_X0) / WP_KX   # ~13.3τ
    # Transmitted wave reaches x=2 at ~t=6-8τ (leading edge of wavepacket)
    # Seed during transmission peak
    seed_time = 8.0                                   # during first transmission peak
    seed_step = int(seed_time / DT)

    # Max time: enough for particles to travel from seed to screen + generous margin
    post_seed_travel = (SCREEN_X - SEED_X) / WP_KX
    T_MAX = seed_time + post_seed_travel * 12.0  # 12× post-seed travel time
    ...
    # Particle arrays (not yet seeded)
    rng = np.random.default_rng(seed=42)
    # Uniform y₀ across both slit apertures: slit1=[+3,+5], slit2=[-5,-3]
    # Equal particles per slit
    half = N_ENSEMBLE // 2
    y0_slit1 = rng.uniform(SLIT_SEP/2 - SLIT_WIDTH/2,
                           SLIT_SEP/2 + SLIT_WIDTH/2, half)
    y0_slit2 = rng.uniform(-SLIT_SEP/2 - SLIT_WIDTH/2,
                           -SLIT_SEP/2 + SLIT_WIDTH/2, N_ENSEMBLE - half)
    part_y0_np = np.concatenate([y0_slit1, y0_slit2])
    rng.shuffle(part_y0_np)  # randomize order
    ...
    print(f"  Seed time: {seed_time:.1f}τ (step {seed_step}), x={SEED_X:.1f}ξ")
    print(f"  Post-seed travel: {post_seed_travel:.1f}τ")
    ...
    for step in range(1, n_steps + 1):
        ...
        # Seed particles at seed_time
        if step == seed_step and not seeded:
            px[:] = SEED_X
            py[:] = torch.tensor(part_y0_np, device=device, dtype=DTYPE)
            active[:] = True
            seeded = True
            for i in range(n_traj):
                traj_x_list[i].append(px[i].item())
                traj_y_list[i].append(py[i].item())
            print(f"  Seeded {N_ENSEMBLE} particles at x={SEED_X:.1f}ξ, "
                  f"y₀ ~ Uniform(−{JITTER_RANGE},+{JITTER_RANGE})ξ, "
                  f"t={seed_time:.1f}τ", flush=True)
        ...
        # Early termination after seeding
    ...
    if len(y_hits) >= 20:
        ...
        # Only correlate bins where there's signal
    ...
    if r_sq is not None and r_sq > 0.5 and len(y_hits) >= 50:
        ...
        conclusion = ("No clear Born-rule emergence → hypothesis falsified "
                      f"(R² = {r_sq:.4f}, N_hits = {len(y_hits)})"
                      if r_sq is not None else
                      f"Insufficient hits ({len(y_hits)}) for analysis")
    ...
    # Panel B: Ensemble histogram + |ψ|² overlay
    ax = axes[1]
    if len(y_hits) > 0:
        ax.hist(y_hits, bins=N_BINS, range=(-y_range, y_range),
                color='#1f77b4', edgecolor='black', alpha=0.8, density=True,
                label=f'Bohmian arrivals (N={len(y_hits)}/{N_ENSEMBLE})')
        # Overlay: |ψ|² scaled to histogram
    ...
    ax.set_title(f'Ensemble Histogram (N={N_ENSEMBLE} trajectories, '
                 f'y₀ ~ Uniform)',
                 fontsize=13, fontweight='bold')
    ax.set_xlim(-y_range, y_range)
    ax.legend(fontsize=11)
    ...
    # ── Initial y₀ distribution plot ──
    fig2, ax2 = plt.subplots(figsize=(8, 4))
    ax2.hist(part_y0_np, bins=50, range=(-SLIT_SEP/2-SLIT_WIDTH, SLIT_SEP/2+SLIT_WIDTH),
    ...
    jd = {
        'parameters': {
            ...
            'N_ENSEMBLE': N_ENSEMBLE, 'JITTER_RANGE': JITTER_RANGE,
