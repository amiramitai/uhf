#!/usr/bin/env python3
"""UHF Phase 12 Night Run: True Merger Edition
λ=10 (down from 50), N=80 (higher resolution), stronger cores, tighter
initial separation. Goal: actual plunge merger with chiral ringdown.

Built on proven Phase 11/12 infrastructure (FFT Poisson, lapse=sqrt(1+2V),
sponge, KO dissipation, tanh K5 saturation, torsion-covariant derivative,
RK2 midpoint, Z4c damping). Single RTX 3090.
"""
...
# ====================== PARAMETERS ======================
N = 80                # Higher resolution (80^3 = 512K cells, ~4GB VRAM)
L = 40.0
dx = L / N            # 0.5
dt = 0.0025           # Smaller dt for finer grid CFL
G_grav = 1.0
m_scalar = 0.1
lam = 10.0            # LOW: allow actual plunge & merger
kappa_torsion = 0.8
m_torsion_sq = 1.0
z4c_damping = 0.05    # Slightly stronger damping for stability
lam_K5_cubic = 0.3
K5_sat = 2.0
KO_sigma = 0.02
n_steps = 50000
...
# Binary IC parameters — tuned for actual merger
d_sep = 3.5           # VERY close initial separation
sigma_core = 2.0      # Tighter cores
amp_core = 0.25       # Much stronger cores — more gravitational binding
v_orbit = 0.5         # Orbital boost
...
# GW extraction shell (wider for finer grid)
...
# Binary cores with orbital momentum (counter-rotating phase gradients)
phi = (phi1 * torch.exp(1j * k_boost * Y) +
       phi2 * torch.exp(-1j * k_boost * Y)).to(torch.complex64)
pi_field = torch.zeros_like(phi)

# Torsion vortex seeds centered on each core
...
def compute_rhs(phi_s, pi_s, K5_s, pi_K5_s):
    ...
    # Torsion-covariant derivative: D²φ = ∇²φ + 2iK5·∇φ - K5²φ
...
print(f"\n{'='*70}")
print(f"  UHF PHASE 12 NIGHT RUN: TRUE MERGER (lambda={lam})")
...
for step in range(n_steps):
    ...
    if step > 0 and step % checkpoint_every == 0:
        with open(f"UHF_results/phase12nr_history_step{step}.json", "w") as f:
            json.dump(history, f)
...
elapsed = time.time() - start_time
print(f"\n{'='*70}")
print(f"  PHASE 12 NIGHT RUN COMPLETE: {step+1} steps in {elapsed:.1f}s ({elapsed/3600:.2f}h)")
print(f"{'='*70}")

with open("UHF_results/phase12nr_history_final.json", "w") as f:
    json.dump(history, f)

# Save CSV time-series
with open("UHF_results/phase12nr_h_plus.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["t","h_plus"])
    for ti, hp in zip(history["t"], history["h_plus"]): w.writerow([ti, hp])
with open("UHF_results/phase12nr_h_cross.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["t","h_cross"])
    for ti, hx in zip(history["t"], history["h_cross"]): w.writerow([ti, hx])
with open("UHF_results/phase12nr_polarization.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["t","f_circ"])
    for ti, fc in zip(history["t"], history["circ_pol"]): w.writerow([ti, fc])
with open("UHF_results/phase12nr_separation.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["t","binary_sep"])
    for ti, sep in zip(history["t"], history["binary_sep"]): w.writerow([ti, sep])
...
# Late ringdown polarization (last 20% of run)
...
axes[1, 1].set_title('Central Lapse')
...
plt.suptitle(f'UHF Phase 12 Night Run: True Merger (lam={lam}, N={N})', fontsize=15, y=1.01)
plt.tight_layout()
plt.savefig("UHF_results/phase12nr_merger_final.png", dpi=300, bbox_inches='tight')
print(f"Plot saved to UHF_results/phase12nr_merger_final.png")
